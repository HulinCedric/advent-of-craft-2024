...
public class SantaCommunicatorTests
{
    ...
    public void ComposeMessage()
        => _communicator.ComposeMessage(new Message(new ReindeerName(Dasher), NorthPole, 5, NumberOfDayBeforeChristmas))
            .Should()
    ...
    public void ShouldDetectOverdueReindeer()
    {
        var overdue = _communicator.IsOverdue(
            new Message(new ReindeerName(Dasher), NorthPole, NumberOfDayBeforeChristmas, NumberOfDayBeforeChristmas),
            _logger);
    ...
    public void ShouldReturnFalseWhenNoOverdue()
        => _communicator.IsOverdue(
                new Message(new ReindeerName(Dasher), NorthPole, NumberOfDayBeforeChristmas - NumberOfDaysToRest - 1, NumberOfDayBeforeChristmas),
                _logger)
            .Should()
            .BeFalse();
}